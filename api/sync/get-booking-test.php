<?php
// api/sync/get-booking-test.php - Fetch single booking from TEST API and save to database
...
try {
    ...
    // Call Holiday Taxis TEST API - GET /bookings/{bookingRef}
    $apiUrl = HolidayTaxisConfig::getApiEndpoint(true) . "/bookings/" . urlencode($bookingRef);

    $headers = [
        "API_KEY: " . HolidayTaxisConfig::getApiKey(true),
        "Content-Type: application/json",
        "Accept: application/json",
        "VERSION: " . HolidayTaxisConfig::getApiVersion(true)
    ];
    ...
    error_log("TEST API Response Keys: " . json_encode(array_keys($detailData)));
    ...
    error_log("TEST API - Extracted status: " . ($status ?? 'NULL'));
    error_log("TEST API - General keys: " . json_encode(array_keys($general)));
    ...
    if (!$status) {
        ...
        error_log("TEST API - Status validation failed. Debug info: " . json_encode($debugInfo));
    ...
    $combinedRawData = [
        ...
        'sync_type' => 'single-booking-fetch-test-api',
    ...
    // Return success response with FULL API data
    echo json_encode([
        'success' => true,
        'data' => [
            'booking_ref' => $bookingRef,
            ...
            'booking_data' => $detailData, // Keep for backward compatibility
            'full_api_response' => $detailData, // Full API response for complete data access
            'raw_api_response' => $response, // Original JSON string from API
            'api_environment' => 'test' // Indicate this is from test environment
        ],
